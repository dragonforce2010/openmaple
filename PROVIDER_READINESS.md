...
| Provider path | Status | What exists today | Required evidence before production use |
|---|---|---|---|
| local_docker | Runnable locally | `ensureDockerRuntime` starts a per-session Docker container, mounts the session workspace, and executes shell/file tools when the environment selects `local_docker`. The default Compose control-plane trial sets `MAPLE_SANDBOX_PROVIDER=local_docker` for local Docker sandbox evaluation. | Host isolation policy, image hardening, network policy, and cleanup strategy. |
