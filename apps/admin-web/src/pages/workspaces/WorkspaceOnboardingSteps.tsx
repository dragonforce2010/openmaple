      ...
      <>
        ...
        <div className="cfg-cards">
          {CLOUD_PROVIDERS.map((provider) => {
            ...
            return <button key={provider.id} type="button" className={`prov-card ${connected ? "on" : ""} ${provider.enabled ? "" : "disabled"}`} disabled={!provider.enabled} onClick={() => provider.enabled && props.setConnectedCloudProviders(connected ? props.connectedCloudProviders.filter((id) => id !== provider.id) : [...props.connectedCloudProviders, provider.id])}>
        ...
        </div> : <div className="panel-empty">{L("请选择至少一个可接入云厂商。Runtime Provider 必须由云厂商提供。", "Choose at least one connectable cloud provider. Runtime provider must come from a cloud provider.")}</div>}
      </>
    );
  }
  if (props.step === 2) {
